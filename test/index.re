...
let connOpts = Mysql.Connection.options(~host=config.host, ~user=config.user, ~password=config.password, ~database=config.database, ());
let conn = Mysql.Connection.make(connOpts);
let poolOpts = Mysql.Pool.options(connOpts);
let pool = Mysql.Pool.make(poolOpts);

let doInsert = () =>
    Mysql.Queryable.query(conn,
        "INSERT INTO test (str, i, b, f, d, dt)
